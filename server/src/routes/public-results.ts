...
router.get('/api/results', async (_req: Request, res: Response) => {
  try {
    const [total, byIsland, byAgeGroup, bySector] = await Promise.all([
    ...
    // Dynamic question stats (anonymized)
    const questions = await pool.query(
      'SELECT id, type, label, options FROM questions WHERE active = true ORDER BY sort_order ASC'
    );

    const questionStats = [];
    for (const q of questions.rows) {
      const stats: any = { id: q.id, type: q.type, label: q.label };
      ...
      if (q.type === 'scale') {
        ...
        // Only show count for text responses (no PII)
        const count = await pool.query(
          'SELECT COUNT(*) as count FROM responses WHERE question_id = $1',
          [q.id]
        );
        stats.response_count = parseInt(count.rows[0].count);
      }
